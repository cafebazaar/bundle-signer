...
import static ir.cafebazaar.apksig.apk.ApkUtils.computeSha256DigestBytes;
import static ir.cafebazaar.apksig.internal.apk.ApkSigningBlockUtils.VERSION_APK_SIGNATURE_SCHEME_V2;
import static ir.cafebazaar.apksig.internal.apk.ApkSigningBlockUtils.VERSION_APK_SIGNATURE_SCHEME_V3;
import static ir.cafebazaar.apksig.internal.apk.ApkSigningBlockUtils.VERSION_JAR_SIGNATURE_SCHEME;

import ir.cafebazaar.apksig.apk.ApkFormatException;
import ir.cafebazaar.apksig.apk.ApkUtils;
import ir.cafebazaar.apksig.internal.apk.ApkSigningBlockUtils;
import ir.cafebazaar.apksig.internal.apk.ContentDigestAlgorithm;
import ir.cafebazaar.apksig.internal.apk.SignatureAlgorithm;
import ir.cafebazaar.apksig.internal.apk.stamp.V2SourceStampSigner;
import ir.cafebazaar.apksig.internal.apk.v1.DigestAlgorithm;
import ir.cafebazaar.apksig.internal.apk.v1.V1SchemeConstants;
import ir.cafebazaar.apksig.internal.apk.v1.V1SchemeSigner;
import ir.cafebazaar.apksig.internal.apk.v1.V1SchemeVerifier;
import ir.cafebazaar.apksig.internal.apk.v2.V2SchemeSigner;
import ir.cafebazaar.apksig.internal.apk.v3.V3SchemeSigner;
import ir.cafebazaar.apksig.internal.apk.v4.V4SchemeSigner;
import ir.cafebazaar.apksig.internal.apk.v4.V4Signature;
import ir.cafebazaar.apksig.internal.jar.ManifestParser;
import ir.cafebazaar.apksig.internal.util.AndroidSdkVersion;
import ir.cafebazaar.apksig.internal.util.Pair;
import ir.cafebazaar.apksig.internal.util.TeeDataSink;
...
import ir.cafebazaar.apksig.util.RunnablesExecutor;
...
public class DefaultApkSignerEngine implements ApkSignerEngine {
    ...
    private DefaultApkSignerEngine(
        ...
        if (v1SigningEnabled && !signerConfigsDisable) {
            if (v3SigningEnabled) {
    ...
    private void createV1SignerConfigs(List<SignerConfig> signerConfigs, int minSdkVersion)
        ...
        if ( mV1ContentDigestAlgorithm == null) {
            mV1ContentDigestAlgorithm = v1ContentDigestAlgorithm;
        }
    ...
    public DigestAlgorithm getV1ContentDigestAlgorithm() {
        return mV1ContentDigestAlgorithm;
    }
    ...
    public V1SchemeSigner.OutputManifestFile outputManifestFile()
            throws ApkFormatException, SignatureException,
        ...
        V1SchemeSigner.OutputManifestFile manifestFile = V1SchemeSigner.generateManifestFile(
                mV1ContentDigestAlgorithm, mOutputJarEntryDigests, inputJarManifest);
        return manifestFile;
    }

    public OutputJarSignatureRequest outputSignatureV1(V1SchemeSigner.OutputManifestFile manifestFile)
    ...
    public void setAddV1SignatureRequest(ApkSignerEngine.OutputJarSignatureRequest outputJarSignatureRequest) {
        mAddV1SignatureRequest = (OutputJarSignatureRequestImpl)outputJarSignatureRequest;
    }

    public void setAddSigningBlockRequest(byte[] apkSigingBlock, int paddingBefore) {
        mAddSigningBlockRequest =
                new OutputApkSigningBlockRequestImpl(apkSigingBlock, paddingBefore);
        mAddSigningBlockRequest.done();
    }
    ...
    public Map<ContentDigestAlgorithm, byte[]> getContentDigestsV2V3Cafebazaar(
        ...
        return getContentDigestsV2V3Cafebazaar(zipEntries, zipCentralDirectory, zipEocd, true);
    }

    private Map<ContentDigestAlgorithm, byte[]> getContentDigestsV2V3Cafebazaar(
        ...
        Map<ContentDigestAlgorithm, byte[]> contentDigests =
            ApkSigningBlockUtils.computeContentDigestsCafebazaar(
                    mExecutor,
                    beforeCentralDir,
                    zipCentralDirectory,
                    eocd);
    ...
    public byte[] signContentDigestsV2V3Cafebazaar(
            Map<ContentDigestAlgorithm, byte[]> contentDigests)
            throws IOException, InvalidKeyException, SignatureException, NoSuchAlgorithmException {
        return signContentDigestsV2V3Cafebazaar(contentDigests, true);
    }
    ...
    private byte[] signContentDigestsV2V3Cafebazaar(
        ...
        if (mV2SigningEnabled) {
            ...
            v2SigningSchemeBlockAndDigests =
                    V2SchemeSigner.generateApkSignatureSchemeV2BlockCafebazaar(
                            contentDigests,
                            v2SignerConfigs,
                            mV3SigningEnabled);
            signingSchemeBlocks.add(v2SigningSchemeBlockAndDigests.signingSchemeBlock);
        }
        // create APK Signature Scheme V3 Signature if requested
        if (mV3SigningEnabled) {
            ...
            v3SigningSchemeBlockAndDigests =
                    V3SchemeSigner.generateApkSignatureSchemeV3BlockCafebazaar(
                            contentDigests,
                            v3SignerConfigs);
    ...
    public OutputApkSigningBlockRequest2 addSignedContentDigestsV2V3Cafebazaar(
            DataSource zipEntries, byte[] apkSigningBlock)
            throws IOException, InvalidKeyException, SignatureException, NoSuchAlgorithmException {
        return addSignedContentDigestsV2V3Cafebazaar(zipEntries, true, apkSigningBlock);
    }

    private OutputApkSigningBlockRequestImpl addSignedContentDigestsV2V3Cafebazaar(
            DataSource zipEntries,
            boolean apkSigningBlockPaddingSupported,
    ...
    private static class OutputJarSignatureRequestImpl implements OutputJarSignatureRequest, Serializable {
    ...
    public static class Builder {
        ...
        public Builder(List<SignerConfig> signerConfigs, int minSdkVersion, boolean signerConfigsDisable) {
            if (signerConfigs.isEmpty() && !signerConfigsDisable) {
                throw new IllegalArgumentException("At least one signer config must be provided");
            }
            if (signerConfigs.size() > 1) {
        ...
        public Builder setV1ContentDigestAlgorithm(
                DigestAlgorithm v1ContentDigestAlgorithm) {
            if (v1ContentDigestAlgorithm != null) {
                mV1ContentDigestAlgorithm = v1ContentDigestAlgorithm;
            }
