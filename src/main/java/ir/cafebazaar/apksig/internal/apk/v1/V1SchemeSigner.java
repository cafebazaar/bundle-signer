...
package ir.cafebazaar.apksig.internal.apk.v1;

import static ir.cafebazaar.apksig.internal.pkcs7.AlgorithmIdentifier.getSignerInfoDigestAlgorithmOid;
import static ir.cafebazaar.apksig.internal.pkcs7.AlgorithmIdentifier.getSignerInfoSignatureAlgorithm;
...
import ir.cafebazaar.apksig.internal.jar.ManifestWriter;
import ir.cafebazaar.apksig.internal.jar.SignatureFileWriter;
...
public abstract class V1SchemeSigner {
    ...
    public static class OutputManifestFile {
        ...
        public Attributes mainSectionAttributes = new Attributes();

        public OutputManifestFile() {}

        public OutputManifestFile(OutputManifestFileSerilizable outputManifestFile) {
            this.contents = outputManifestFile.contents;
            this.individualSectionsContents = outputManifestFile.individualSectionsContents;

            HashMap<String, String> mainSectionAttributes = outputManifestFile.mainSectionAttributes;

            Set<String> keys = mainSectionAttributes.keySet();

            for (String key : keys) {
                String value = mainSectionAttributes.get(key);
                this.mainSectionAttributes.putValue(key, value);
            }
        }
    }

    public static class OutputManifestFileSerilizable implements Serializable {
        public byte[] contents;
        public SortedMap<String, byte[]> individualSectionsContents;
        public HashMap<String, String> mainSectionAttributes = new HashMap<String, String>();

        public OutputManifestFileSerilizable(OutputManifestFile outputManifestFile) {
            this.contents = outputManifestFile.contents;
            this.individualSectionsContents = outputManifestFile.individualSectionsContents;

            Attributes mainSectionAttributes = outputManifestFile.mainSectionAttributes;

            Set<Object> keys = mainSectionAttributes.keySet();

            for (Object key : keys) {
                Object value = mainSectionAttributes.get(key);
                this.mainSectionAttributes.put(key.toString(), value.toString());
            }
        }
